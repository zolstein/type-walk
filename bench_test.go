package type_walk_test

import (
	"bytes"
	"github.com/stretchr/testify/require"
	tw "github.com/zolstein/type-walk"
	"math/rand/v2"
	"reflect"
	"runtime"
	"strconv"
	"testing"
)

func BenchmarkSimpleJsonSerialize(b *testing.B) {

	type Inner struct {
		D int
		E string
	}

	type Outer struct {
		A string
		B int
		C []*Inner
	}

	toSerialize := make([]*Outer, 100)
	for i := range toSerialize {
		inners := make([]*Inner, rand.N(10))
		for j := range inners {
			inners[j] = &Inner{
				D: rand.Int(),
				E: randString(10),
			}
		}
		toSerialize[i] = &Outer{
			A: randString(20),
			B: rand.Int(),
			C: inners,
		}
	}

	var bb bytes.Buffer
	var intBuf []byte

	var serializeReflect func(v reflect.Value)
	serializeReflect = func(v reflect.Value) {
		switch v.Kind() {
		case reflect.Int:
			intBuf = intBuf[:0]
			intBuf = strconv.AppendInt(intBuf, v.Int(), 10)
			bb.Write(intBuf)
		case reflect.String:
			bb.WriteRune('"')
			bb.WriteString(v.String())
			bb.WriteRune('"')
		case reflect.Ptr:
			if v.IsNil() {
				bb.WriteString("null")
			} else {
				serializeReflect(v.Elem())
			}
		case reflect.Slice:
			bb.WriteRune('[')
			for i := range v.Len() {
				if i > 0 {
					bb.WriteRune(',')
				}
				serializeReflect(v.Index(i))
			}
			bb.WriteRune(']')
		case reflect.Struct:
			bb.WriteRune('{')
			t := v.Type()
			for i := range t.NumField() {
				if i > 0 {
					bb.WriteRune(',')
				}
				sf := t.Field(i)
				serializeReflect(reflect.ValueOf(sf.Name))
				bb.WriteRune(':')
				serializeReflect(v.Field(i))
			}
			bb.WriteRune('}')
		default:
			panic("unhandled default case")
		}
	}

	serializeReflect(reflect.ValueOf(toSerialize))

	b.Run("reflect", func(b *testing.B) {
		runtime.GC()
		b.ResetTimer()
		b.ReportAllocs()
		for range b.N {
			bb.Reset()
			serializeReflect(reflect.ValueOf(toSerialize))
			bs := bb.Bytes()
			_ = bs
		}
	})

	register := tw.NewRegister[*bytes.Buffer]()
	register.RegisterCompileIntFn(func(r reflect.Type) tw.WalkFn[*bytes.Buffer, int] {
		return func(ctx *bytes.Buffer, i *int) error {
			intBuf = intBuf[:0]
			intBuf = strconv.AppendInt(intBuf, int64(*i), 10)
			bb.Write(intBuf)
			return nil
		}
	})
	register.RegisterCompileStringFn(func(r reflect.Type) tw.WalkFn[*bytes.Buffer, string] {
		return func(ctx *bytes.Buffer, s *string) error {
			bb.WriteRune('"')
			bb.WriteString(*s)
			bb.WriteRune('"')
			return nil
		}
	})
	register.RegisterCompileSliceFn(func(r reflect.Type) (tw.SliceWalkFn[*bytes.Buffer], error) {
		return func(ctx *bytes.Buffer, s tw.SliceWalker[*bytes.Buffer]) error {
			bb.WriteRune('[')
			for i := range s.Len() {
				if i > 0 {
					bb.WriteRune(',')
				}
				err := s.Walk(ctx, i)
				if err != nil {
					return err
				}
			}
			bb.WriteRune(']')
			return nil
		}, nil
	})
	register.RegisterCompileStructFn(func(r reflect.Type, sfr tw.StructFieldRegister[*bytes.Buffer]) (tw.StructWalkFn[*bytes.Buffer], error) {
		fields := make([]reflect.StructField, r.NumField())
		for i := range fields {
			i, err := sfr.RegisterField(i)
			if err != nil {
				return nil, err
			}
			fields[i] = r.Field(i)
		}
		return func(ctx *bytes.Buffer, s tw.StructFieldWalker[*bytes.Buffer]) error {
			bb.WriteRune('{')
			for i := range s.NumFields() {
				if i > 0 {
					bb.WriteRune(',')
				}
				bb.WriteRune('"')
				bb.WriteString(fields[i].Name)
				bb.WriteRune('"')
				bb.WriteRune(':')
				err := s.Walk(ctx, i)
				if err != nil {
					return err
				}
			}
			bb.WriteRune('}')
			return nil
		}, nil
	})
	register.RegisterCompilePtrFn(func(r reflect.Type) (tw.PtrWalkFn[*bytes.Buffer], error) {
		return func(ctx *bytes.Buffer, p tw.PtrWalker[*bytes.Buffer]) error {
			if p.IsNil() {
				bb.WriteString("null")
			}
			return p.Walk(ctx)
		}, nil
	})

	walker := tw.NewWalker(register)
	typeWalker, err := tw.NewTypeWalker[*bytes.Buffer, []*Outer](walker)
	require.NoError(b, err)

	serializeTypeWalk := func(toSerialize *[]*Outer) {
		bb.Reset()
		err := typeWalker.Walk(&bb, toSerialize)
		require.NoError(b, err)
	}

	b.Run("type-walk", func(b *testing.B) {
		toSerializePtr := &toSerialize
		runtime.GC()
		b.ResetTimer()
		b.ReportAllocs()
		for range b.N {
			serializeTypeWalk(toSerializePtr)
			bs := bb.Bytes()
			_ = bs
		}
	})
}

func randString(length int) string {
	var letters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
	bytes := make([]byte, rand.N(length))
	for i := range bytes {
		bytes[i] = letters[rand.N(len(letters))]
	}
	return string(bytes)
}
